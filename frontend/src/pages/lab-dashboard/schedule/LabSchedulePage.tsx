...
const LOCAL_SLOTS_KEY = "megacare_lab_local_slots";
const LOCAL_SCHEDULE_KEY = "megacare_lab_schedule";
...
export default function LabSchedulePage() {
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    setDataLoading(true);
    try {
      const res = await fetch("/api/lab/appointments", { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
  ...
  useEffect(() => {
    if (!isLoading && isAuthenticated && user?.role === "lab_radiology") fetchAppointments();
  }, [isLoading, isAuthenticated, user, fetchAppointments]);

  useEffect(() => {
    if (!isLoading && (!isAuthenticated || !user || user.role !== "lab_radiology")) navigate("/login");
  ...
  if (isLoading || !isAuthenticated || !user || user.role !== "lab_radiology") return null;
  ...
  const confirmSlot = async (dk: string, time: string) => {
    ...
    if (slot?.appointmentId) {
      const token = localStorage.getItem("megacare_token");
      await fetch(`/api/lab/appointments/${slot.appointmentId}`, {
        method: "PUT",
