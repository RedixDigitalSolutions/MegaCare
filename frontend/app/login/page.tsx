...
export default function LoginPage() {
  ...
  const roles = [
    { key: "patient" as const, label: "Patient", desc: "Consultations en ligne", Icon: FaUserAlt, color: "from-blue-500 to-cyan-500" },
    { key: "doctor" as const, label: "Médecin", desc: "Gérer vos patients", Icon: FaUserMd, color: "from-emerald-500 to-teal-500" },
    { key: "pharmacy" as const, label: "Pharmacien", desc: "Gestion du stock", Icon: FaPills, color: "from-green-500 to-lime-500" },
    { key: "medical_service" as const, label: "Services Médicaux", desc: "Hospitalisation à domicile", Icon: FaHospital, color: "from-purple-500 to-indigo-500" },
    { key: "lab_radiology" as const, label: "Labos & Radiologie", desc: "Analyses & Imagerie", Icon: FaMicroscope, color: "from-rose-500 to-pink-500" },
    { key: "medical_transport" as const, label: "Transport Médicalisé", desc: "Ambulances & transport", Icon: FaAmbulance, color: "from-orange-500 to-red-500" },
    { key: "paramedical" as const, label: "Paramédicaux", desc: "Infirmiers & thérapeutes", Icon: FaUserNurse, color: "from-sky-500 to-blue-500" },
  ];
          ...
          <div className="relative space-y-8">
            <div>
              ...
              <h2 className="text-3xl xl:text-4xl font-bold text-white leading-tight mb-4">
                Votre santé,<br />connectée et sécurisée
              </h2>
              <p className="text-white/70 text-sm leading-relaxed">
                Accédez à des médecins spécialistes, gérez vos prescriptions et suivez votre parcours de santé depuis un seul endroit.
              </p>
            </div>
            <div className="space-y-3">
              {["Consultations vidéo 24h/24", "Dossier médical sécurisé", "Pharmacie en ligne", "Transport médicalisé"].map((f) => (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mb-8">
              {roles.map(({ key, label, desc, Icon, color }) => (
                ...
                >
                  ...
                  <p className="font-semibold text-foreground text-sm group-hover:text-foreground leading-tight mb-1">{label}</p>
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="hidden lg:flex lg:w-2/5 xl:w-1/3 relative flex-col justify-between p-10 bg-gradient-to-br from-primary via-primary/90 to-accent overflow-hidden">
        ...
        <div className="relative space-y-6">
          <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${activeRole.color} bg-white/10 flex items-center justify-center`}>
          ...
          <div className="space-y-3">
            {["Accès sécurisé à votre espace", "Données chiffrées de bout en bout", "Conforme aux normes de santé"].map((f) => (
        ...
        <div className="w-full max-w-md">
          <button
            onClick={() => {
              setUserRole(null);
              setAuthError(null);
              setFormData({ email: "", password: "", doctorId: "", pharmacyId: "", serviceId: "", labId: "", transportId: "", paramedicalId: "", companyName: "" });
            }}
          ...
          <div className="mb-8">
            ...
            <h1 className="text-3xl font-bold text-foreground mb-1">Connexion</h1>
            <p className="text-muted-foreground text-sm">Bienvenue. Entrez vos identifiants pour continuer.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isLoading || !formData.email || !formData.password || (userRole === "doctor" && !formData.doctorId) || (userRole === "pharmacy" && !formData.pharmacyId)}
