  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="relative w-20 h-20 rounded-xl overflow-hidden border border-border bg-secondary/30 flex items-center justify-center">
        {imageUrl ? <img src={imageUrl} alt={name} className="w-full h-full object-contain p-1" /> : <Package size={28} className="text-muted-foreground/40" />}
      </div>
      {mode === "idle" && (
...
function AddToStockModal({ onClose, onAdded }: { onClose: () => void; onAdded: () => void }) {
  ...
  const [manualForm, setManualForm] = useState<Omit<CatalogResult, "id">>({
  ...
  const handleSelectItem = (item: CatalogResult) => {
    setSelected(item);
    setStep("price-stock");
  };
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-muted/30">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-foreground">
              ...
              {step === "manual" && "Nouveau produit paramédical"}
            ...
            <div className="p-4 border-b border-border space-y-3">
              <div className="relative">
                ...
                <input
                  ...
                  type="text"
                  autoFocus
                  placeholder="Tapez pour chercher (nom, marque, catégorie)..."
                  value={query}
              ...
              <div className="flex flex-wrap gap-1.5">
                ...
                {["Orthopédie", "Aide à la mobilité", "Rééducation", "Ergonomie", "Respiratoire", "Soins"].map((c) => (
                  <button
                    key={c}
              ...
              <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                <span>
                  {query.trim()
                    ...
                    : `${popular.length} produits populaires`
                  }
                </span>
            ...
            <div ref={listRef} className="flex-1 overflow-y-auto p-3 space-y-1">
              {query.trim().length >= 1 && !searching && results.length === 0 && (
                <div className="text-center py-10">
                  ...
                  <p className="text-xs text-muted-foreground mb-5">Ce produit n'existe pas encore dans le catalogue global.</p>
                  <button
              ...
              {showingPopular && popular.length > 0 && (
                <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider px-1 pb-1">Produits populaires</p>
              )}
              ...
              {displayList.map((item, i) => (
                ...
                >
                  <div className="w-9 h-9 rounded-lg overflow-hidden border border-border bg-secondary/30 flex items-center justify-center shrink-0">
                    {item.imageUrl ? <img src={item.imageUrl} alt={item.name} className="w-full h-full object-contain p-0.5" /> : <Package size={16} className="text-muted-foreground/40" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {item.prescription && <span className="px-1.5 py-0.5 text-[10px] font-semibold bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400 rounded">ORD</span>}
                    </div>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {item.category && <span className={`px-1.5 py-0.5 text-[10px] font-medium rounded ${getCatColor(item.category)}`}>{item.category}</span>}
                      <span className="text-[11px] text-muted-foreground truncate">
                        <HighlightText text={item.brand || ""} query={query} />
                      </span>
                    </div>
                  </div>
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            <div className="p-6 overflow-y-auto flex-1 space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-medium text-muted-foreground mb-1 block">Nom du produit *</label>
                  <input type="text" value={manualForm.name} onChange={(e) => setManual("name", e.target.value)} placeholder="ex: Attelle de poignet" className="w-full px-3 py-2 text-sm bg-background border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50" />
                </div>
                <div>
                ...
                <div>
                  <label className="text-xs font-medium text-muted-foreground mb-1 block">Marque / Fabricant</label>
                  <input type="text" value={manualForm.brand} onChange={(e) => setManual("brand", e.target.value)} placeholder="ex: Thuasne" className="w-full px-3 py-2 text-sm bg-background border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50" />
                </div>
                <div className="flex items-end">
                  <label className="flex items-center gap-2 cursor-pointer text-sm">
                    <input type="checkbox" checked={manualForm.prescription} onChange={(e) => setManual("prescription", e.target.checked)} className="w-4 h-4 accent-primary rounded" />
                    <ShieldAlert size={14} className="text-amber-500" /> Ordonnance requise
                  </label>
                </div>
                <div className="md:col-span-2">
                  <label className="text-xs font-medium text-muted-foreground mb-1 block">Description</label>
                  <textarea rows={2} value={manualForm.description} onChange={(e) => setManual("description", e.target.value)} placeholder="Description du produit..." className="w-full px-3 py-2 text-sm bg-background border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none" />
                </div>
                <div className="md:col-span-2">
                  <label className="text-xs font-medium text-muted-foreground mb-1 block">Usage / Instructions</label>
                  <textarea rows={2} value={manualForm.usage} onChange={(e) => setManual("usage", e.target.value)} placeholder="Instructions d'utilisation..." className="w-full px-3 py-2 text-sm bg-background border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none" />
                </div>
              </div>
            </div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-4 p-4 bg-primary/5 border border-primary/20 rounded-xl">
              <div className="w-14 h-14 rounded-xl overflow-hidden border border-border bg-secondary/30 flex items-center justify-center shrink-0">
                {selected.imageUrl ? <img src={selected.imageUrl} alt={selected.name} className="w-full h-full object-contain p-1" /> : <Package size={24} className="text-muted-foreground/40" />}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-foreground">{selected.name}</span>
                  {selected.prescription && <span className="px-1.5 py-0.5 text-[10px] font-semibold bg-amber-100 text-amber-700 rounded">ORD</span>}
                </div>
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="text-xs text-muted-foreground truncate">{selected.brand || ""}</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" min={0} step={0.01} value={price} onChange={(e) => setPrice(e.target.value)} placeholder="ex: 45.00" className="w-full px-3 py-2.5 text-sm bg-background border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50" autoFocus />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground mb-1 block">Quantité en stock *</label>
                <input type="number" min={0} value={stock} onChange={(e) => setStock(e.target.value)} placeholder="ex: 50" className="w-full px-3 py-2.5 text-sm bg-background border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50" />
              </div>
            </div>

            <p className="text-xs text-muted-foreground">
              Le prix et la quantité sont propres à votre établissement. Les informations du produit sont partagées dans le catalogue global.
            </p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-5">
          <div className="flex justify-center">
            <ImageEditor imageUrl={form.imageUrl} name={form.name} onChange={(url) => set("imageUrl", url)} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
            ...
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Prix (DT) *</label>
            ...
            <div className="md:col-span-2">
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Description</label>
              <textarea rows={2} value={form.description} onChange={(e) => set("description", e.target.value)} className="w-full px-3 py-2 text-sm bg-background border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none" />
            </div>
            <div className="md:col-span-2">
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Usage / Instructions</label>
              <textarea rows={2} value={form.usage} onChange={(e) => set("usage", e.target.value)} className="w-full px-3 py-2 text-sm bg-background border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none" />
            </div>
          </div>
        </div>
        <div className="flex gap-3 px-6 py-4 border-t border-border bg-muted/20">
...
export default function ParamedicalStockPage() {
  ...
  useEffect(() => {
    if (isAuthenticated && user?.role === "paramedical") fetchProducts();
  ...
  if (isLoading || !isAuthenticated || !user || user.role !== "paramedical") return null;
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
                <input type="text" placeholder="Rechercher par nom, marque..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2.5 text-sm bg-card border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50" />
              </div>
              <div className="relative">
                    ...
                    <tbody className="divide-y divide-border">
                      {filtered.map((p) => (
                        <tr key={p.id} className="hover:bg-muted/30 transition group">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-lg overflow-hidden border border-border bg-secondary/30 flex items-center justify-center shrink-0">
                                {p.imageUrl ? <img src={p.imageUrl} alt={p.name} className="w-full h-full object-contain p-0.5" /> : <Package size={18} className="text-muted-foreground/40" />}
                              </div>
                          ...
                          <td className="px-4 py-3">
                            {p.category ? <span className={`text-xs px-2 py-1 rounded-md font-medium ${getCatColor(p.category)}`}>{p.category}</span> : <span className="text-xs text-muted-foreground">—</span>}
                          </td>
