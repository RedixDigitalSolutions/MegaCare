...
export default function LabAppointmentsPage() {
    ...
    useEffect(() => {
        (async () => {
            try {
                const res = await fetch("/api/appointments/facility?type=lab", {
            ...
            <main className="flex-1 p-4 md:p-8 overflow-y-auto">
                <div className="max-w-4xl mx-auto space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-foreground">Laboratoires & Radiologie</h1>
                            <p className="text-sm text-muted-foreground">Vos rendez-vous dans les laboratoires et centres de radiologie</p>
                        </div>
                    </div>
                    ...
                    {shown.length === 0 ? (
                        <div className="text-center py-20 space-y-3 bg-card border border-border rounded-2xl">
                            ...
                            <p className="text-sm text-muted-foreground">
                                {tab === "upcoming"
                                    ? "Vos rendez-vous avec les laboratoires apparaîtront ici."
                                        ...
                                        <div key={appt.id}>
                                            ...
                                            {isExpanded && (
                                                <div className="px-5 pb-4 bg-secondary/10 border-t border-border space-y-2">
                                                    {appt.service && (
                                                        <p className="text-sm mt-3"><span className="font-medium">Examen :</span> {appt.service}</p>
                                                    )}
                                                    {appt.notes && (
                                                    ...
                                                    {appt.adminNotes && (
                                                        <p className="text-sm text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/30 px-3 py-2 rounded-lg">
                                                            <span className="font-medium">Message du laboratoire :</span> {appt.adminNotes}
                                                        </p>
                                                    )}
                                                </div>
                                            )}
