...
export default function ParamedicalProductPage() {
  ...
  type ProductDetail = ParamedicalProduct & {
    description: string;
    usage: string;
    compatibility: string;
    features: string[];
              ...
              <div className="flex gap-3">
                {product.images.map((img, i) => (
                  <button
                    key={i}
                    onClick={() => setImgIdx(i)}
                    className={`w-20 h-20 rounded-xl overflow-hidden border-2 transition ${imgIdx === i
                      ? "border-primary"
          ...
          <div className="flex gap-1 border-b border-border mb-6">
            {(
              ...
              <button
                ...
                className={`px-5 py-3 text-sm font-semibold border-b-2 -mb-px transition-colors ${activeTab === key
