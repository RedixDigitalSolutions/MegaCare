      ...
      .then((d) => {
        const list = Array.isArray(d)
          ? d.map((doc) => ({
            ...
            initials: String(doc.name || "").split(" ").map((w: string) => w[0]).join("").slice(0, 2).toUpperCase(),
          }))
          : [];
        setDoctors(list);
      })
                ...
                <div ref={chatRef} className="flex-1 overflow-y-auto p-4 space-y-3">
                  {messages.map((m, i) => (
                    <div key={i} className={`flex ${m.from === "me" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[80%] rounded-xl px-3 py-2 text-sm ${m.from === "me"
              ...
              {filtered.map((doc) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-card rounded-xl border p-4 transition ${isSelected ? "border-primary bg-primary/5" :
